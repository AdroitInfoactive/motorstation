	...
	function validate(){
		...
		if(optn == 'p'){
			...
			if(document.frmsrch.chkexact.checked==true){
				document.frmsrch.action="vw_all_city.php?optn=p&val="+val+"&chk=y";
				document.frmsrch.submit();
			}
			else{
				document.frmsrch.action="vw_all_city.php?optn=p&val="+val;
				document.frmsrch.submit();
			}
		}		
		else if(optn == 'c'){
			    var val = document.frmsrch.lstcnty.value;
				document.frmsrch.action="vw_all_city.php?optn=c&val="+val;
				document.frmsrch.submit();
		}
		else if(optn == 'y'){
			    var val = document.frmsrch.lstcntry.value;
				document.frmsrch.action="vw_all_city.php?optn=y&val="+val;
				document.frmsrch.submit();
		}
		return true;
	}
      ...
      <tr>
	  	<td height="400" valign="top">
			  <table  width="95%" border="0" align="center" cellpadding="5" cellspacing="1" bgcolor="#FFFFFF">
				 <tr class='white'>
					<td height="26" colspan="4" bgcolor="#FF543A"><span class="heading"><strong>:: City </strong></span><br>
					</td>
				</tr>
              <tr bgcolor="#f0f0f0">
			  	<td colspan="6" border="0" cellspacing="1" cellpadding="5">
				   <form method="POST" action="" name="frmsrch" id="frmsrch" onSubmit="return validate()">
					...
					<!--<td width="40%">-->
						 <div id="div1" style="display:block;float:left;padding-left:70px;">
						 ...
						 <div id="div2" style="display:none;float:left;padding-left:70px;">
						...
						<div id="div3" style="display:none;float:left;padding-left:70px;">
						  ...
						  <div style="float:left;padding-left:70px">
							 ...
							 <a href="vw_all_city.php" class="leftlinks">Refresh</a>
							 <!--</td>
							 <td width="17%">-->
							</div>
							 <div align="right" style="float:left;padding-left:70px">
						<?php						
						/*if($gaddflg == 1)
						{*/
						?>
						  <input name="btn2" type="button" class="" id="btn2" onClick="addnew()" value="&laquo; Add">
						  <?php
						//}
						?>
						  </div>
			 ...
			 <tr  bgcolor="#f0f0f0">
              ...
              <tr class="white" >
               <td width="6%" align="left" bgcolor="#FF543A"><strong>Sl.No.</strong></td>
              	<td width="17%" bgcolor="#FF543A" align="left"><strong>City Name</strong></td>
				<td width="16%" bgcolor="#FF543A" align="left"><strong>County</strong></td>
                ...
                <td width="5%" bgcolor="#FF543A" align="right"><strong>Rank</strong></td>    
				   <?php
					/*if($gedtflg == 1)
					{*/
					?>
                <td width="6%" align="center" bgcolor="#FF543A"><strong>Edit</strong></td>
			...
			<tr <?php if($cnt%2==0){echo "bgcolor='f9f8f8'";}else{echo "bgcolor='#F2F1F1'";}?>>
				...
				<td bgcolor="#f0f0f0" align="left"><a href="edit_city.php?vw=<?php echo $rowscty_mst['ctym_id'];?>&pg=<?php echo $pgnum;?>&countstart=<?php echo $cntstart.$loc;?>" class="leftlinks"><?php echo $rowscty_mst['ctym_name'];?></a></td>
                <td bgcolor="#f0f0f0" align="left"><?php echo $rowscty_mst['cntym_name'];?></td>
				<td bgcolor="#f0f0f0" align="left"><?php echo $rowscty_mst['cntrym_name'];?></td>
				<td bgcolor="#f0f0f0" align="left"><?php echo $rowscty_mst['ctym_iso'];?></td>
				<td bgcolor="f0f0f0" align="right"><?php echo $rowscty_mst['ctym_prty'];?></td>	
					 <?php
					/*if($gedtflg == 1)
					{*/
					?>
					<td align="center" bgcolor="#f0f0f0" >
					<a href="edit_city.php?vw=<?php echo $rowscty_mst['ctym_id'];?>&pg=<?php echo $pgnum;?>&countstart=<?php echo $cntstart.$loc;?>" class="leftlinks">Edit</a></td>
					<td align="center" bgcolor="#f0f0f0">
					<input type="checkbox" name="chksts"  id="chksts" value="<?php echo $rowscty_mst['ctym_id'];?>" <?php if($rowscty_mst['ctym_sts'] =='a') { echo "checked";}?> onClick="addchkval(<?php echo $rowscty_mst['ctym_id'];?>,'hidchksts','frmtsk','chksts');">				</td>
					<?php
					/*}
					if($gdelflg == 1)
					{*/
				...
				<?php
				$disppg = funcDispPag('paging',$loc,$sqrycty_mst1,$rowsprpg,$cntstart, $pgnum, $conn);	
					if($disppg != ""){	
