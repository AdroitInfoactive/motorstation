<?php
	  ...
	  /***************************************************************/
		...
		if(($_POST['hidchksts']!="") && isset($_REQUEST['hidchksts'])){
			...
			$updtsts = funcUpdtAllRecSts('pncd_mst','gnrlenqrym_id',$id,'pncdm_sts');		
			if($updtsts == 'y'){
		...
		if(($_POST['hidchkval']!="") && isset($_REQUEST['hidchkval'])){
			...
			$delsts = funcDelAllRec('gnrlenqry_mst','gnrlenqrym_id',$did);	
			if($delsts == 'y'){
	  	...
	  	<td height="400" valign="top">
			...
			<input type="hidden" name="hidchksts" id="hidchksts">	
			  <table  width="95%" border="0" align="center" cellpadding="5" cellspacing="1" bgcolor="#FFFFFF">
				 <tr class='white'>
					<td height="26" colspan="4" bgcolor="#FF543A"><span class="heading"><strong>:: General Enquiries</strong></span><br>
				   </td>
				</tr>
				<tr valign="top">
			...
			$sqrypncd_mst1="select 
								gnrlenqrym_id,gnrlenqrym_name,gnrlenqrym_emailid, date_format(gnrlenqrym_crtdon,'%d-%m-%Y %h:%i:%s') as gnrlenqrym_crtdon
						   from 
								gnrlenqry_mst
						   where
								gnrlenqrym_id != '' ";
			  if(isset($_REQUEST['txtsrchcd']) && (trim($_REQUEST['txtsrchcd'])!="")){
				  ...
				  if(isset($_REQUEST['chk']) && (trim($_REQUEST['chk'])=='y')){
						$sqrypncd_mst1.=" and gnrlenqrym_emailid='$txtsrchcd'";
					}
					else{
						$sqrypncd_mst1.=" and gnrlenqrym_emailid like '%$txtsrchcd%'";
					}		
				}
				 if(isset($_REQUEST['txtsrchnm']) && (trim($_REQUEST['txtsrchnm'])!="")){
				  ...
				  if(isset($_REQUEST['chk']) && (trim($_REQUEST['chk'])=='y')){
						$sqrypncd_mst1.=" and gnrlenqrym_name='$txtsrchnm'";
					}
					else{
						$sqrypncd_mst1.=" and gnrlenqrym_name like '%$txtsrchnm%'";
					}		
				}
			...
			$sqrypncd_mst = $sqrypncd_mst1." order by gnrlenqrym_crtdon limit $offset,$rowsprpg";
			...
			<tr <?php if($cnt%2==0){echo "bgcolor='#f9f8f8'";}else{echo "bgcolor='#F2F1F1'";}?>>
				...
				<?php /*?>	<td align="center" bgcolor="#f0f0f0">
					<input type="checkbox" name="chksts"  id="chksts" value="<?php echo $rowspncd_mst['gnrlenqrym_id'];?>" <?php if($rowspncd_mst['pncdm_sts'] =='a') { echo "checked";}?> onClick="addchkval(<?php echo $rowspncd_mst['gnrlenqrym_id'];?>,'hidchksts','frmtsk','chksts');">				</td><?php */?>
					<?php
					/*}
					if($gdelflg == 1)
					{*/
					?>
					<td align="center">
					<input type="checkbox" name="chkdlt"  id="chkdlt" value="<?php echo $rowspncd_mst['gnrlenqrym_id'];?>"></td>
					<?php
					//}
					?>
				  </tr>
