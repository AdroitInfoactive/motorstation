<?php
	...
	*/
	...
	function  funcDispPagTxt($cls,$rdval,$qrystr,$recprpg,$cntstart,$pgnum,$distinct,$fldnm){ 		
		...
		if(($mxpg > 0) && ($pgnum != 1)){
				$prvval	 = $pgnum - 1;
				$prvspg .= "<a class='$cls' href=\"$self?pg=$prvval$loc\"><img src='/images/pre_arrow.png' width='22' height='22' alt='Go To Previous Page'/></a>";		
		}		
		if(($mxpg > 1) && ($pgnum != $mxpg)){
				$nxtval	 = $pgnum + 1;
				$nxtpg  .= "<a class='$cls' href=\"$self?pg=$nxtval$loc\"><img src='/images/next_arrow.png' width='22' height='22' alt='Go To Next Page'/></a>";		
		}
		if($pgnum<=$mxpg){
			for($j=$cntstart+1; $j < $cntstart+11; $j++){
				if($j!=$pgnum && $j<=$mxpg){
